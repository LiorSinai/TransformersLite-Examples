...
"""
function confusion_matrix(ŷ::AbstractVector, y::AbstractVector, labels::AbstractVector)
...
function recall(cm::AbstractMatrix; target::Int=1)
    tp = cm[target, target]
    tp / sum(cm[target, :])
end

function precision(cm::AbstractMatrix; target::Int=1)
    tp = cm[target, target]
    fp = sum(cm[:, target]) - tp
    tp / (tp + fp)
end

function f1_score(cm::AbstractMatrix; target::Int=1)
